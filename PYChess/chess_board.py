...
class Board:
    def __init__(self,screen,cell_w,cell_h):
        ...
        self.image_map_init()
        self.board_init()
        self.image_render(screen,black = [242, 156, 63],white = [255, 255, 255])
    ...
    def image_render(self,screen,black,white):
        ##filling colors
        for i in range(8):
            for j in range(8):
                if(i%2 == j%2):
                    self.cells[i][j].fill_color(screen,white)
                else:
                    self.cells[i][j].fill_color(screen,black)
        
        #rendering images
        for i in range(8):
            for j in range(8):
                self.cells[i][j].render(screen,self.get_image(i,j))
